...
// Implementación del botón de instalación de la PWA
...
// El botón del Wake Lock ahora solo cambia el estado
...
function speakAsync(text, langCode, voiceURI = null) {
...
async function renderAndPlay() {
    ...
    if (showTranslation) {
        ...
        await speakAsync(card[transKey] || '', resolveLangCode(transKey), transVoiceSel.value);
        if (isPaused) return;
    ...
    for (let i = 0; i < repeatCount; i++) {
        ...
        await speakAsync(card[studyKey] || '', resolveLangCode(studyKey), voiceToUse);
        if (isPaused) return;
        if (i < repeatCount - 1) {
